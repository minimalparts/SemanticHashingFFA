...
def run_pca(lang=None, spf=None):
    ...
    for p in grid:
        ...
        score = wiki_cat_purity(lang=lang, spf=spf, m=pca_m, logprob_power=p['logprob_power'], top_words=p['top_words'],
                                num_nns=20, metric="cosine", verbose=False)
        scores.append(score)
...
def hack_pca_model(lang=None, spf=None, logprob_power=None, top_words=None, input_m=None, pca_m=None):
    ...
    for a in alphas:
        ...
        score = wiki_cat_purity(lang=lang, spf=spf, m=ridge_m, logprob_power=logprob_power, top_words=top_words,
                                num_nns=20, metric="cosine", verbose=False)
        scores.append(score)
...
def train_umap(data=None, spf=None, logprob_power=7, umap_nns=20, umap_min_dist=0.0, umap_components=31):
    ...
    param_grid = {'logprob_power': [4, 6], 'top_words': [300, 500], 'umap_nns': [15, 20],
                  'umap_min_dist': [0.2, 0.6, 0.9], 'umap_components': [16, 32]}
    ...
    for p in grid:
        ...
        umap_model = umap.UMAP(n_neighbors=p['umap_nns'], min_dist=p['umap_min_dist'],
                               n_components=p['umap_components'], metric='hellinger', random_state=32,
                               verbose=False).fit(input_m)
        umap_m = umap_model.transform(input_m)
        score = wiki_cat_purity(lang=data, spf=spf, m=umap_m, logprob_power=p['logprob_power'],
                                top_words=p['top_words'], num_nns=100, metric="cosine", verbose=False)
        scores.append(score)
    ...
    p = grid[best]
    input_m, _ = vectorize_scale(data, spf, p['logprob_power'], p['top_words'])
    umap_model = umap.UMAP(n_neighbors=p['umap_nns'], min_dist=p['umap_min_dist'], n_components=p['umap_components'],
                           metric='hellinger', random_state=32).fit(input_m)
    umap_m = umap_model.transform(input_m)
    umap_m = np.nan_to_num(umap_m)
    wiki_cat_purity(lang=data, spf=spf, m=umap_m, logprob_power=p['logprob_power'], top_words=p['top_words'],
                    num_nns=20, metric="cosine", verbose=True)
...
def hack_umap_model(data=None, spf=None, logprob_power=None, top_words=None, input_m=None, umap_m=None):
    ...
    for a in alphas:
        ...
        ridge_m = ridge.predict(input_m)
        score = wiki_cat_purity(lang='en', spf=spf, m=ridge_m, logprob_power=logprob_power, top_words=top_words,
                                num_nns=20, metric="cosine", verbose=False)
        scores.append(score)
        print("HACKED UMAP SCORE:", score, a)
...
def train_fly(data=None, spf=None, logprob_power=None, top_words=None, umap=True, num_trials=None, kc_size=None,
              k=None):
    ...
    for p in grid:
        ...
        fly_list = [Fly(pn_size, kc_size, p['wta'], p['proj_size'], top_words, init_method, eval_method, proj_store,
                        hyperparameters) for _ in range(num_trials)]
