...
def generate_cluster_labels(data=None, spf=None, labels=None, logprob_power=None, top_words=None, verbose=True):
    print('--- Generating cluster labels for Birch model ---')
    spm_vocab = f"./spm/{data}/spm.{data}.vocab"
    ...
    m, titles, _ = vectorize(data, spf, logprob_power, top_words)
    ...
    file_path = spf.replace('.sp','.cluster.labels.pkl')
